...
points if there HAG is below 0.0. I added an assign filter to set the Z (HAG) value for points below 0.0 to
0.0 and the empty cells were eliminated.

As with the HAG metrics, I suspect the interpolation process is causing the bulk of the difference. The remainder
is due to the right and top edge points in cells in SilviMetric but this should be small differences.
...
For completeness, I created another CHM using SilviMetric using the points normalized by FUSION.
